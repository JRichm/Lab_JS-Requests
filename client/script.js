...
sayHelloButton.addEventListener('mouseover', hoverFunction);
...
const ohMy = () => {
    // YOUR CODE HERE
    axios.get('http://localhost:4000/animals').then(result => {
        let obj = 0;
        result.data.forEach(() => {
            let newP = document.createElement('p');
            newP.textContent = String(result.data[obj]);
            document.querySelector('body').appendChild(newP);
            obj++;
        })
    }).catch(err => console.log({err, message: "Problem with ohMy"}));
}
...
// CODE HERE
let newFunction = () => {
    axios.get(`http://localhost:4000/query-test?query=${Math.floor(Math.random() * 100)}&2`).then(result => {
...
function createFood(e) {
    ...
    axios.post(`http://localhost:4000/food`, body).then(result => {
        let newFoodItem = document.createElement('p');
        newFoodItem.innerHTML = foodInput.value;
        document.getElementById('foods').appendChild(newFoodItem);
        foodInput.value = '';
    }).catch(err => console.log({err, message: "Problem with createFood"}));
}

document.getElementById('input-form').addEventListener("submit", (e) => createFood(e));